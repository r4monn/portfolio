  ...
  return (
    <div className="h-full bg-primary/60 xl:bg-primary/30 py-32 text-center xl:text-left">
      <motion.div
        ...
        animate="show"
        exit="hidden"
        className="hidden xl:flex absolute bottom-0 -left-[620px]"
      >
        <Avatar />
      </motion.div>
          ...
          <div className="flex justify-evenly gap-x-4 xl:gap-x-8 mx-auto xl:mx-0 mb-4">
            {aboutData.map((item, itemIndex) => {
              return (
                <div 
                  key={itemIndex} 
                  className={`${index === itemIndex && 'text-violet-500 after:w-[100%] after:bg-violet-500 after:transition-all after:duration-300'} 
                ...
                <div key={itemIndex} className="flex-1 flex flex-col md:flex-row max-w-max gap-x-2 items-center text-white/90">
                  ...
                  <div className="flex gap-x-4">
                    {item.icons?.map((icon, itemIndex) => {
                      return <div className="text-4xl lg:text-6xl">{icon}</div>
                    })}
                  </div>
