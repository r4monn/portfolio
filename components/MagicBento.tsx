...
const DEFAULT_GLOW_COLOR = '136, 206, 2';
...
const MagicBento: React.FC<BentoProps> = ({
  ...
  const DEFAULT_CARD_COLOR = theme === 'dark' ? '#000300' : 'white';
  const DEFAULT_BORDER_COLOR = theme === 'dark' ? '#132015' : '#E8E8E8';
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {cardData.map((card, index) => {
            const baseClassName = `card flex flex-col justify-center relative aspect-[4/2] min-h-[220px] w-full max-w-full p-5 rounded-[20px] border border-solid font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${
              enableBorderGlow ? 'card--border-glow' : ''
            }`;

            const cardStyle = {
              backgroundColor: DEFAULT_CARD_COLOR || 'red',
              ...
              return (
                ...
                >
                  ...
                  <div className="card__content flex flex-col relative text-foreground">
                    ...
                    <p
                      className={`card__description text-foreground/65 text-sm leading-5 opacity-90 ${textAutoHide ? 'text-clamp-2' : ''}`}
                    >
                      {card.description}
                    </p>
                  </div>
